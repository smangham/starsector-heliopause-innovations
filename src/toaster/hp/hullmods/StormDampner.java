...
public class StormDampner implements HullModEffect {
    public static final String ID = HullMods.STORM_DAMPNER;  /// The game ID for this hullmod.
    public static final String KEY = "$"+Tags.STORM_DAMPNER;  /// The memory key for a fleet with a storm dampner.
    public static final String TAG = Tags.STORM_DAMPNER;  /// The tag for a ship with a storm dampner.
    ...
    public static FleetMemberAPI getFleetMemberWithDampner(CampaignFleetAPI fleet) {
        ...
        for (FleetMemberAPI member: fleet.getFleetData().getMembersListCopy()) {
            ...
            for (String hullModId : member.getHullSpec().getBuiltInMods()) {
                if (Global.getSettings().getHullModSpec(hullModId).hasTag(TAG)) return member;
            }
        }
        return null;
    }
